...
//
// 1. no whitespace around =>

/**
 * Sniffs_Blobfolio_WhiteSpace_OperatorSpacingSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Blobfolio_Sniffs_WhiteSpace_OperatorSpacingSniff implements PHP_CodeSniffer_Sniff
{
	...
	public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr) {
		...
		//double arrow should not have spaces
		if ($tokens[$stackPtr]['code'] === T_DOUBLE_ARROW) {
			if ($tokens[($stackPtr - 1)]['code'] === T_WHITESPACE) {
				$found = $tokens[($stackPtr - 1)]['length'];
				$error = "Expected no space before \"$operator\"; $found found";
				$fix   = $phpcsFile->addFixableError($error, $stackPtr, 'NoSpaceBefore');
				if ($fix === true) {
					$phpcsFile->fixer->replaceToken(($stackPtr - 1), '');
				}
				$phpcsFile->recordMetric($stackPtr, 'No space before double-arrow', $found);
			}

			if ($tokens[($stackPtr + 1)]['code'] === T_WHITESPACE) {
				$found = $tokens[($stackPtr + 1)]['length'];
				$error = "Expected no space after \"$operator\"; $found found";
				$fix   = $phpcsFile->addFixableError($error, $stackPtr, 'NoSpaceBefore');
				if ($fix === true) {
					$phpcsFile->fixer->replaceToken(($stackPtr + 1), '');
				}
				$phpcsFile->recordMetric($stackPtr, 'No space after double-arrow', $found);
			}
